...
class TorBridge:
    ...
    async def connect_onion(self, onion: str, port: int = 9230, simulate_censorship: bool = False) -> bool:
        """Conecta a outro nó ARKHE via .onion."""
        if simulate_censorship:
            print(f"  [TOR] Censura total detectada. Falha ao conectar em {onion}:{port}")
            return False
    ...
    def get_transport_config(self) -> Dict:
        ...
        return {
            ...
            "transport_priority": ["tor", "quic", "nostr"],
        }
...
class NostrTorIpfsBridge:
    ...
    async def mesh_connect(self, target_onion: Optional[str] = None, simulate_censorship: bool = False) -> bool:
        """Conecta à malha via TOR ou NOSTR fallback."""
        if target_onion:
            success = await self.tor.connect_onion(target_onion, simulate_censorship=simulate_censorship)
            if success:
                return True
            print("  [972.1] TOR falhou. Acionando failover para ponte NOSTR...")
